...
jQuery(document).ready(function() {
    ...
    $.getJSON('recipes.json', function(data) {
    	...
    	$.each(data.recipes, function(key, recipe) {
    		recipeList.push('<div class="recipe-item col-md-4 col-sm-6 ');
    		for (var i in recipe.categories) {
    			recipeList.push(recipe.categories[i] + ' ');
    		}
    		recipeList.push('"><div class="panel panel-default"><div class="panel-heading">' + recipe.title + '</div><div class="panel-body"><p><img class="card-icon" src="img/users.svg">serves: <strong>' + recipe.serves + '</strong></p><p><img class="card-icon" src="img/clock.svg">prep: <strong>' + recipe.prep + '</strong></p><p><img class="card-icon" src="img/clock.svg">wait: <strong>' + recipe.wait + '</strong></p><button type="button" class="btn btn-default" data-toggle="modal" data-designation="' + recipe.designation + '" data-target="#recipe-modal">View Recipe</button></div></div></div>');
    	});
